...
export function createIsolatedBackendEnv() {
  ...
  // O CMS tem storage próprio em runtime. Mantemos os nomes legados para os
  // módulos compartilhados durante o corte, mas os prefixos CMS garantem que
  // este teste nunca caia no storage real do backend público.
  process.env.CMS_STORAGE_ROOT = storageRoot;
  process.env.CMS_UPLOADS_DIR = uploadsDir;
  ...
  process.env.CMS_RATE_LIMITS_STORE_PATH = path.join(privateRoot, "cms-rate-limits.json");
