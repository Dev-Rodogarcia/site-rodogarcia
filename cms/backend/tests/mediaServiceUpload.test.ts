...
describe("mediaService upload processing", () => {
  ...
  it("requires confirmation before deleting media in use and clears its references", async () => {
    ...
    fs.cpSync(path.join(repoRoot, "site", "frontend", "public"), env.publicDir, { recursive: true });
    fs.copyFileSync(
      path.join(repoRoot, "site", "backend", "storage", "content.json"),
      path.join(env.storageRoot, "content.json")
    );
    fs.copyFileSync(
      path.join(repoRoot, "site", "backend", "storage", "site-texts.json"),
