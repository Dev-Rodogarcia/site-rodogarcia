...
export function isSupremeUser(user: UserRecord | null | undefined): boolean {
  return isSupremeCmsUser(user);
}
...
export function createUser(params: CreateUserParams, actor?: UserRecord) {
  // Não delegamos criação de identidades: mesmo uma conta "comum" pode ser
  // reconfigurada depois, portanto o ciclo completo pertence ao supremo.
...
export function changeOwnPassword(
  ...
  if (!updated) throw new HttpError(404, "Usuário não encontrado.");
  if (currentSessionId) {
    sessionRepository.deleteByUserIdExcept(user.id, currentSessionId);
  } else {
    // Chamadas de serviço sem uma sessão corrente segura revogam tudo.
