...
import { requireCmsPermission, requireSupremeCmsUser } from "../security/cmsAccess.js";
...
import {
  ...
  deleteLandingMediaController,
  getLandingPreviewController,
  listLandingMediaController,
  listLandingPagesController,
  publishLandingPageController,
  updateLandingPageController,
  uploadLandingMediaController,
} from "../controllers/landingBuilderController.js";
...
adminRouter.get("/access-profiles", requireSupremeCmsUser, listAccessProfilesController);
adminRouter.post("/access-profiles", requireSupremeCmsUser, requireAllowedOrigin, requireJson, requireCsrf, createAccessProfileController);
adminRouter.put("/access-profiles/:id", requireSupremeCmsUser, requireAllowedOrigin, requireJson, requireCsrf, updateAccessProfileController);
adminRouter.delete("/access-profiles/:id", requireSupremeCmsUser, requireAllowedOrigin, requireCsrf, deleteAccessProfileController);
...
adminRouter.get("/landings/:id/preview", getLandingPreviewController);
adminRouter.get("/landing-media", listLandingMediaController);
adminRouter.post(
  "/landing-media",
  requireAllowedOrigin,
  requireCsrf,
  requireContentLengthLimit(70 * 1024 * 1024),
  landingMediaUpload.single("file"),
  uploadLandingMediaController
);
adminRouter.delete(
  "/landing-media/:id",
...
adminRouter.post(
  ...
  createAdminImprovementController
);
