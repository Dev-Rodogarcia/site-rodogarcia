...
- O client ESL serializa chamadas de saída do processo com intervalo mínimo de 2 segundos, pois todas compartilham o mesmo IP público. As mutações públicas da integração também compartilham rate limit local de 10 tentativas por IP/hora e validam origem e JSON antes de chamar o fornecedor.
- Em `/cotacao`, carga fracionada chama o BFF e exibe o total calculado; carga fechada recebe a mensagem compilada para o canal comercial. Origem e destino aceitam CEP, com preenchimento de cidade/UF pelo ViaCEP, ou cidade configurada na Base de unidades, com CEP genérico cadastrado. Em `/coletas`, a NF precisa ser validada antes do agendamento e a confirmação é invalidada se algum CNPJ de contexto ou dado da NF for alterado; o sucesso mostra o número de coleta e o cadastro de cliente inválido encaminha o atendimento sem expor a resposta técnica do ESL.
...
- Cotação fracionada cria `quoteCreate` sempre com a tabela de preço `PADRÃO` e devolve `quoteStretchBids.total` por trecho e total consolidado. O CNPJ de cliente também é enviado como pagador; remetente e destinatário são opcionais. A filial é escolhida por cidade e o backend resolve seu CNPJ a partir de `units.quoteCnpj`, sem expor o valor ao navegador. Altura, largura e comprimento são apoios opcionais para calcular o m³, que também pode ser preenchido diretamente. Carga fechada não chama o ESL: o backend valida o mesmo contrato e devolve a mensagem compilada para WhatsApp.
- Coleta consulta a NF no ESL antes de retornar o `invoiceId`; na criação, consulta novamente a NF com chave/número, remetente e destinatário e rejeita validação expirada ou divergente. O `invoiceId` não é persistido internamente. O retorno de cliente não cadastrado prepara atendimento no WhatsApp sem vazar erro técnico. Atualização e cancelamento usam exclusivamente o `id` remoto; o cancelamento aceita motivo canônico, exige complemento para `OUTROS` e atribui `cancellationDatetime` no servidor.
- Os atalhos de coleta da Home, CMS, busca e rodapé apontam para `/coletas`; o conteúdo canônico de quick actions e footer também usa essa rota. Links de WhatsApp só abrem o aplicativo quando `external.whatsappCommercial`, `external.whatsappQuoteFractional` ou `external.whatsappQuoteFull` receberem URL oficial `https://wa.me/...` ou `https://api.whatsapp.com/...`; até essa configuração, a interface preserva a mensagem compilada para cópia e direciona ao contato institucional.
...
### Integração ESL — Configuração operacional

- [ ] Preencher na Base de unidades do CMS o `CNPJ para cotação` e o `CEP genérico da cidade` de cada filial que deve aparecer no seletor público de `/cotacao`. Apenas unidades ativas com CNPJ válido são disponibilizadas; o conteúdo canônico inicial contém a matriz de Agudos/SP como referência.
- [ ] Informar e configurar as URLs oficiais do WhatsApp comercial, de carga fracionada e de carga fechada em `frontend/src/lib/routes.ts`. As URLs atuais são o fallback de contato institucional; sem números oficiais não é seguro inventar o destino do WhatsApp.
