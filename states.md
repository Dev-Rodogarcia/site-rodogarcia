...
- A leitura de `/api/admin/seo-settings` agora tolera referências legadas ou inexistentes de imagem social e usa o fallback WebP do projeto, evitando que uma mídia removida bloqueie a tela de SEO. O salvamento continua aceitando somente arquivos internos existentes da Biblioteca de mídia.
- As ajudas de `/developer/analytics` agora explicam de forma específica a Configuração, a Auditoria, as Conversões, as páginas mais acessadas e a contagem de eventos. Filtros e colunas da tabela de auditoria também descrevem os eventos, rotas, datas e sessões anônimas; provedores externos só carregam após consentimento de Analytics.
...
- O bloco `Exemplo real` foi removido das interrogações do CMS. As ajudas agora mostram somente o `Resumo` e os detalhes aplicáveis ao controle, eliminando exemplos genéricos repetidos.
- Os fallbacks de ajuda usam perfis de todas as rotas do CMS e categorias de campo. Seções de botão, mídia e FAQ explicam sua ação; campos de título, texto, link, mídia, cor, status, contato e regras de tempo recebem resumo e destino específicos ao tipo.
...
- Em `/developer/rastreamento`, os Registros recentes usam vinte linhas por página; o card ocupa toda a altura da coluna e sua paginação fica ancorada ao rodapé, alinhada à paginação de Auditoria.
...
- Uploads administrativos de imagem não mantêm mais o arquivo de origem em `/uploads`: PNG, JPG/JPEG, AVIF e WebP são validados e gravados somente como variantes WebP otimizada, thumbnail, medium e large; vídeos continuam no formato original compatível.
- `backend/scripts/migrate-images-to-webp.js` permite auditar a conversão sem escrita e, com `--apply`, transforma imagens legadas PNG/JPG/JPEG/AVIF de `frontend/public` e `backend/storage/uploads`, atualiza referências em conteúdo e código e remove cada original somente após validar o WebP. A migração aplicada converteu 34 imagens locais e reduziu o total de 65.408.970 para 3.912.280 bytes.
- Vídeos enviados pelo CMS são validados e convertidos para WebM (VP9/Opus), sem manter o arquivo de origem público. O backend usa o FFmpeg multiplataforma empacotado por `ffmpeg-static`; `FFMPEG_PATH` é uma sobrescrita privada opcional, não uma dependência do PATH do servidor.
- `backend/scripts/migrate-videos-to-webm.js` oferece simulação e migração de MP4/Ogg legados. A migração aplicada converteu os cinco vídeos locais para WebM, atualizou as referências públicas e reduziu o total de 11.211.145 para 5.720.502 bytes.
- A Biblioteca em `/developer/imagens` recebe todas as mídias com data de arquivo e as mostra da mais recente para a mais antiga; em empate ou data ausente, usa o nome como critério estável.
- A Biblioteca permite excluir somente arquivos enviados ao storage de uploads. Se a mídia estiver em uso, o CMS pede confirmação explícita; confirmada a ação, as referências em conteúdo, slots, popup e SEO são limpas em transação para que os fallbacks públicos assumam, e as variantes do arquivo são removidas com auditoria.
