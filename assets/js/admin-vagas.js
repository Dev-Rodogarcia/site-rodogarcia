...
function renderList() {
  ...
  state.vagas.forEach((item) => {
    ...
    const header = createElement('div', 'item-head');
    const meta = createElement('div', 'item-meta');
    meta.append(
      createElement('strong', '', item.title || 'Sem titulo'),
      createElement('p', '', `${item.location || ''} | ${item.status || ''} | Ordem: ${item.order}`)
    );

    const status = createElement('div', 'status-group');
    status.append(
      createElement('span', `status-pill ${item.active ? 'on' : 'off'}`, item.active ? 'Ativa' : 'Inativa'),
      createElement('span', `status-pill ${item.featured ? 'on' : 'off'}`, item.featured ? 'Destaque' : 'Normal')
    );
    ...
    actions.append(
      createButton('Editar', () => fillForm(item)),
      createButton('Toggle Destaque', async () => {
        await updateItem({ ...item, featured: !item.featured });
      }),
      createButton('Ativar/Desativar', async () => {
        await updateItem({ ...item, active: !item.active });
      }),
      createButton('Subir', async () => {
        await reorder(item.id, -1);
      }),
      createButton('Descer', async () => {
        await reorder(item.id, 1);
      }),
      createButton('Excluir', async () => {
...
function bindForm() {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      renderList();
      resetForm();
      showPanelMessage('Vaga salva com sucesso.', 'success');
    } catch (error) {
      showPanelMessage(error.message, 'error');
    }
  });

  const clearBtn = document.getElementById('vaga-clear-btn');
  if (clearBtn) {
