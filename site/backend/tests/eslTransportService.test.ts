...
function configureEslForTest() {
  ...
  // Mantém o fallback derivado do tenant mesmo quando existe um .env local.
  // dotenv preserva uma variável já definida, inclusive a string vazia.
  process.env.ESL_GRAPHQL_URL = "";
  vi.resetModules();
}
...
describe("integração ESL de cotação e coleta", () => {
  ...
  it("valida a NF pelo ESL com CNPJ vinculado e devolve apenas capability opaca", async () => {
    configureEslForTest();
    ...
    expect(result).toMatchObject({ validated: true, validationToken: expect.any(String) });
    expect(result).not.toHaveProperty("invoice");
    expect(result.validationToken).not.toContain("8950942");
    ...
    expect(sent.variables.params.issuer).toEqual({ document: "01351335000117" });
    expect(sent.variables.params.recipient).toBeUndefined();
  });

  it("recusa a consulta de NF sem vínculo de CNPJ antes de chamar o ESL", async () => {
    configureEslForTest();
    const { parseInvoiceLookupRequest } = await import("../src/validators/eslTransport.js");
    ...
    ).toThrow("Informe o CNPJ do remetente ou do destinatário");
  });

  it("recusa anexar uma NF validada com CNPJ diferente da coleta", async () => {
    configureEslForTest();
    ...
    expect(() =>
      parseCollectionRequest({
        ...collectionPayload,
        senderCnpj: "60960473000162",
        invoice: validatedInvoice,
        invoiceValidationToken: createInvoiceValidationToken(
          "8950942",
          invoiceValidationFingerprint(validatedInvoice)
        ),
      })
    ).toThrow("Os CNPJs da NF devem corresponder");
  });
  ...
  it("agenda a coleta com a filial da origem e usa o cliente também como pagador", async () => {
    ...
    const collectionWithValidatedInvoice = {
      ...collectionPayload,
      senderCnpj: "01351335000117",
      invoice: { ...collectionPayload.invoice, senderCnpj: "01351335000117" },
    };
    const result = await createCollection(
      parseCollectionRequest({
        ...collectionWithValidatedInvoice,
        invoiceValidationToken: createInvoiceValidationToken(
          "8950942",
          invoiceValidationFingerprint(collectionWithValidatedInvoice.invoice)
        ),
      })
    );

    expect(result).toMatchObject({ requiresWhatsApp: false, collection: { id: "359397" } });
    if (result.requiresWhatsApp) throw new Error("A coleta deveria ter sido criada.");
    expect(result.collection.maintenanceToken).toMatch(/^v1\./);
    ...
    expect(pickItem.sender).toEqual({ document: "01351335000117" });
