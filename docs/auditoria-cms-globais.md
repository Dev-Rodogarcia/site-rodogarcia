...
Não foi encontrado, neste escopo, vazamento de segredo, senha, hash, cookie ou storage privado para o cliente. As rotas administrativas passam por `requireAdmin`, e as mutações verificadas usam origem, tipo de conteúdo e CSRF (`cms/backend/src/routes/adminRoutes.ts:55-66`, `cms/backend/src/routes/adminRoutes.ts:148-242`). Os problemas principais são de verdade funcional do CMS e consistência dos contratos, não de bypass administrativo.

## Achados prioritários

### P1 — Slots de mídia expostos sem consumidor e fallback impossível de restaurar

O editor declara 25 slots em `cms/frontend/src/app/developer/imagens/page.tsx:59-85` e diz que eles são “usados pelo site” (`cms/frontend/src/app/developer/imagens/page.tsx:303-304`). O rastreamento encontrou:

- 7 slots realmente consumidos hoje: `home.cert.iso`, `home.cert.sassmaq`, `home.cert.ecovadis`, `home.cert.pf`, `home.cert.pcsp`, `home.cert.exercito` e `home.cert.ibama` (`site/frontend/src/app/page.tsx:115-156`).
- 2 slots usados somente por fallback de migração quando o conteúdo moderno não existe: `about.hero` e `careers.culture` (`cms/backend/src/services/pageContent.ts:721-736`, `cms/backend/src/services/pageContent.ts:850-858`).
...
Mesmo nos sete slots ativos, “Usar fallback do site” não funciona depois que um valor já foi salvo. A UI envia string vazia (`cms/frontend/src/app/developer/imagens/page.tsx:518-525`), mas a API CMS elimina pares vazios antes do merge e mantém o valor antigo (`cms/backend/src/services/mediaService.ts:489-502`).

A API CMS também aceita qualquer chave e valida todos os slots como mídia de tipo `all`, sem allowlist nem contrato de tipo por slot (`cms/backend/src/services/mediaService.ts:493-499`). A UI filtra imagem/vídeo, mas a API não preserva essa garantia.
...
A página declara que os registros alimentam diretamente o mapa, o seletor ativo e os dados exibidos (`cms/frontend/src/app/developer/unidades/page.tsx:191-198`, `cms/frontend/src/app/developer/unidades/page.tsx:390-394`). O site, porém, renderiza `homePage.regionalPresence.units`, um segundo conjunto de dados (`site/frontend/src/app/page.tsx:242-248`).

No editor da Home, selecionar uma unidade operacional apenas copia um snapshot de oito campos (`cms/frontend/src/app/developer/home/page.tsx:1017-1032`). Alterações futuras na unidade original não são propagadas. O próprio editor da Home descreve corretamente esse vínculo como “ponto de partida” (`cms/frontend/src/app/developer/home/page.tsx:1755-1768`). `linkedUnitId` não é resolvido na API CMS nem no componente público.
...
- `isDefault` permite múltiplos valores, enquanto o mapa escolhe simplesmente a primeira unidade da Home ordenada (`site/frontend/src/components/home/BrazilMap.tsx:29-53`).
- A rota existe em `admin.units`, mas não participa de `adminNavigationGroups`, portanto fica órfã e recebe contexto visual de Dashboard (`cms/frontend/src/lib/routes.ts:53-74`, `cms/frontend/src/lib/routes.ts:194-232`, `cms/frontend/src/lib/routes.ts:325-347`).
...
- `behavior.blockAnalyticsUntilConsent`: aparece no CMS (`cms/frontend/src/app/developer/lgpd-cookies/page.tsx:193-196`) e é salvo, mas nunca é lido. Analytics permanece sempre bloqueado até a categoria `analytics` ser aceita (`site/frontend/src/components/analytics/AnalyticsProvider.tsx:100-105`, `site/frontend/src/components/analytics/AnalyticsProvider.tsx:257-295`).
- `desktop.position`: possui três opções (`cms/frontend/src/app/developer/lgpd-cookies/page.tsx:200-203`), mas o banner desktop sempre usa o container central fixo (`site/frontend/src/components/analytics/ConsentBanner.tsx:264-273`). O posicionamento central foi consolidado como decisão visual atual; manter um seletor contrário a essa regra é que constitui o campo falso.
- `categories[].required` e `categories[].enabledByDefault`: são toggles visíveis (`cms/frontend/src/app/developer/lgpd-cookies/page.tsx:220-223`), mas o service recalcula ambos exclusivamente a partir de `key === "necessary"` (`cms/backend/src/services/consentService.ts:60-69`). O valor digitado é descartado no save/load.

`mobile.position` é funcional: `center-modal` altera as classes mobile (`site/frontend/src/components/analytics/ConsentBanner.tsx:159-162`). `requireExplicitChoice`, `reopenOnVersionChange`, versão, textos e labels também têm consumidores reais. IDs, IP mascarado, device e logs na tela de monitoramento são dados operacionais legítimos, não campos prometendo mudança no site.

Há ainda uma divergência reproduzível no monitoramento: o cliente registra escolhas customizadas como `partial` (`site/frontend/src/components/analytics/AnalyticsProvider.tsx:492-504`) e a API CMS aceita/consulta `partial` (`cms/backend/src/services/consentService.ts:159-178`), mas o tipo do CMS omite esse valor e o filtro “Parcial” envia `custom` (`cms/frontend/src/app/developer/monitoramento-cookies/page.tsx:27-31`, `cms/frontend/src/app/developer/monitoramento-cookies/page.tsx:129-136`). O teste existente confirma que o contrato real é `partial` (`cms/backend/tests/consentService.test.ts:7-39`).
...
O schema administrativo aceita `siteUrl`, um bloco próprio de consentimento, heartbeat, Sentry e Search Console (`cms/backend/src/services/analyticsService.ts:32-67`). O DTO público expõe somente `tracking`, GA4 e Clarity (`cms/backend/src/services/analyticsService.ts:93-107`). O consumidor público lê `tracking.enabled`, `scrollMilestones`, GA4 e Clarity; não existe heartbeat, Sentry, Search Console ou uso de `siteUrl` (`site/frontend/src/components/analytics/AnalyticsProvider.tsx:26-35`, `site/frontend/src/components/analytics/AnalyticsProvider.tsx:257-336`).
...
`tracking.enabled` é apenas parcial: ele interrompe os eventos internos, mas não participa do efeito que injeta GA4/Clarity (`site/frontend/src/components/analytics/AnalyticsProvider.tsx:257-295`). O rótulo genérico “Tracking ativo” pode ser interpretado como chave mestra de todos os provedores.

As conversões também estão infladas: a API CMS conta `form_submit` como formulário convertido e lead (`cms/backend/src/services/analyticsService.ts:231-242`), embora esse evento seja disparado antes do resultado (`site/frontend/src/components/analytics/AnalyticsProvider.tsx:416-425`) e haja um evento específico `form_success` (`site/frontend/src/components/analytics/AnalyticsProvider.tsx:428-436`).
...
Cada envio de contato ou cotação é salvo no repository legado e também cria um novo registro, com outro ID, no repository central (`cms/backend/src/services/formsService.ts:20-45`, `cms/backend/src/services/formsService.ts:67-96`, `cms/backend/src/services/leadsService.ts:43-103`). A listagem une os quatro repositories e deduplica apenas por ID (`cms/backend/src/services/leadsService.ts:106-134`), portanto os dois registros sobrevivem e aparecem duplicados.

Popup não sofre a mesma duplicação porque reutiliza o objeto/ID central ao gravar o repository específico (`cms/backend/src/services/popupService.ts:141-166`).

Os cards “Popup” e “Formulários” também são calculados apenas sobre a página atual de resultados, enquanto “Total” vem da API CMS global (`cms/frontend/src/app/developer/leads/page.tsx:47-76`, `cms/frontend/src/app/developer/leads/page.tsx:85-93`). A troca de página altera os cards sem que o rótulo avise que é uma amostra.
...
O editor aceita `0` para atraso, cooldown e máximo por sessão (`cms/frontend/src/app/developer/popup-exit/page.tsx:547-587`). O service usa `Number(value) || default`, portanto atraso `0` vira `10`, cooldown `0` vira `24` e máximo `0` vira `1` (`cms/backend/src/services/popupService.ts:77-82`). Para máximo, a regra mínima de 1 é coerente; a divergência é a UI oferecer 0. Para atraso/cooldown, zero é um valor válido prometido e deveria ser preservado.

Os toggles `enableName`, `enableEmail` e `enablePhone` podem ser todos desligados e o save valida apenas título, descrição e botão (`cms/frontend/src/app/developer/popup-exit/page.tsx:242-247`, `cms/frontend/src/app/developer/popup-exit/page.tsx:592-646`). O popup público então renderiza um formulário sem campos, mas a API CMS rejeita qualquer envio sem ao menos um contato (`cms/backend/src/services/popupService.ts:133-139`).

Outras divergências:

- Em falha de `/popup-config`, o componente ativa `DEFAULT_CONFIG`, que vem com `enabled: true`; os listeners são instalados e o comportamento é fail-open (`site/frontend/src/components/exit-popup/ExitPopup.tsx:210-242`, `site/frontend/src/components/exit-popup/ExitPopup.tsx:276-341`).
- A descrição desktop do fallback público contém mojibake (`site/frontend/src/components/exit-popup/ExitPopup.tsx:57-60`).
- Títulos/descrições específicos de desktop/mobile e `sheetTitle` não têm `maxLength` no CMS, mas a API CMS os corta em 120/280/80 caracteres (`cms/frontend/src/app/developer/popup-exit/page.tsx:432-525`, `cms/backend/src/services/popupService.ts:85-112`).
- `popup_ignored` é aceito e exibido nos totais, mas o cliente nunca emite esse evento (`cms/backend/src/services/popupService.ts:46-51`, `cms/backend/src/services/popupService.ts:229-258`, `site/frontend/src/components/exit-popup/ExitPopup.tsx:262-271`, `site/frontend/src/components/exit-popup/ExitPopup.tsx:358-360`).
...
`slug` é editável e descrito como caminho público (`cms/frontend/src/app/developer/seo/page.tsx:277-285`), é persistido (`cms/backend/src/services/seoService.ts:66-83`), mas não integra o DTO público nem altera rota, canonical ou metadata (`site/frontend/src/lib/cmsPublic.ts:5-16`, `site/frontend/src/lib/cmsPublic.ts:26-70`). É um campo sem efeito.

O layout raiz aplica `title.template = "%s | Rodogarcia Transportes"` (`site/frontend/src/app/layout.tsx:22-25`). `buildCmsMetadata` devolve o título CMS como string, portanto recebe o template; ao mesmo tempo, nove defaults do SEO já incluem `| Rodogarcia Transportes` (`cms/backend/src/services/seoService.ts:8-20`). O resultado esperado nessas rotas é marca duplicada. A Home evita isso somente no fallback absoluto, mas uma configuração CMS carregada volta a ser string.

Também há truncamento silencioso: OG Title, OG Description e metatags não têm limites visíveis no editor (`cms/frontend/src/app/developer/seo/page.tsx:291-326`), enquanto a API CMS limita 95/220/1000 (`cms/backend/src/services/seoService.ts:76-83`). `canonical` atualiza `<link rel=canonical>`, mas `openGraph.url` continua usando o path fixo em vez do canonical (`site/frontend/src/lib/cmsPublic.ts:34-61`).
...
O CMS oferece adicionar/remover/ordenar colunas, links inferiores e sociais, e remover horários/blocos (`cms/frontend/src/app/developer/footer-links/page.tsx:173-203`, `cms/frontend/src/app/developer/footer-links/page.tsx:576-662`, `cms/frontend/src/app/developer/footer-links/page.tsx:721-749`). A API CMS, porém, troca arrays vazios pelos defaults:

- colunas, sociais, links inferiores e horários: `cms/backend/src/services/footerLinksContent.ts:434-474`;
- blocos de Termos: `cms/backend/src/services/footerLinksContent.ts:482-514`;
- blocos de Privacidade: `cms/backend/src/services/footerLinksContent.ts:578-604`.
...
- O editor altera label e URL, mas não `external`; o sanitizer preserva o boolean anterior (`cms/backend/src/services/footerLinksContent.ts:62-72`, `cms/frontend/src/app/developer/footer-links/page.tsx:154-169`). Mudar um link externo para interno pode continuar abrindo como externo.
- Ícones sociais e das ações rápidas são texto livre, enquanto o público possui allowlists pequenas e usa fallback (`site/frontend/src/components/layout/FooterSocialLink.tsx:10-29`, `site/frontend/src/app/central-ajuda/page.tsx:32-36`, `site/frontend/src/app/central-ajuda/page.tsx:96-105`). O save “funciona”, mas valores fora da lista não produzem o ícone escolhido.
- O componente genérico aceita 90/220/280 caracteres, mas Termos e cabeçalhos da Ajuda cortam alguns desses campos em 80/160/260/180 na API CMS (`cms/frontend/src/app/developer/footer-links/page.tsx:890-938`, `cms/backend/src/services/footerLinksContent.ts:492-509`, `cms/backend/src/services/footerLinksContent.ts:543-564`).

O cardinality fixo de três ações rápidas e das perguntas da Ajuda é explícito no código/UI (“Pergunta fixa”) e no sanitizer (`cms/frontend/src/app/developer/footer-links/page.tsx:813-849`, `cms/frontend/src/app/developer/footer-links/page.tsx:974-1020`, `cms/backend/src/services/footerLinksContent.ts:532-567`). Isso é uma regra atual legítima, não um campo de ordenação quebrado; apenas não atende uma eventual expectativa de CRUD livre.

### P2 — Rastreamento não exibe o nome das ações de auditoria

A API CMS persiste auditoria em `action` (`cms/backend/src/services/auditService.ts:28-40`). O tipo do CMS frontend não possui `action` e a UI tenta renderizar `event.event || event.type` (`cms/frontend/src/app/developer/rastreamento/page.tsx:20-29`, `cms/frontend/src/app/developer/rastreamento/page.tsx:235-247`). Os cards aparecem com título vazio.

O resumo “Eventos” também não é um total histórico: `getTrackingSummary` força `limit: 1000` (`cms/backend/src/services/trackingService.ts:122-162`), embora o storage aceite mais eventos. O rótulo não informa o limite.
...
O contador de referências e a substituição em lote percorrem apenas `content`, `site-texts` e `media-slots` (`cms/backend/src/services/mediaService.ts:195-200`, `cms/backend/src/services/mediaService.ts:464-478`). Referências guardadas em popup e `seo-settings` não são contadas nem trocadas. Uma imagem usada apenas como OG image ou no popup pode aparecer sem selo “Em uso” e continuar apontando para a URL antiga.

A copy da substituição cita explicitamente `content.json` e `site-texts.json` (`cms/frontend/src/app/developer/imagens/page.tsx:442-447`), portanto não mente sobre esse comando específico; a divergência está no conceito global de biblioteca/uso de mídia. O upload em si está corretamente protegido por validação de MIME, assinatura, limites e variantes. A descrição “gera WebP e thumbnail” deve, contudo, ser condicionada a imagens, pois a mesma entrada aceita vídeo e vídeos não são convertidos (`cms/frontend/src/app/developer/imagens/page.tsx:48-58`, `cms/frontend/src/app/developer/imagens/page.tsx:335-351`).

### P2 — Usuários: permissão descrita difere da regra e update aceita coerção perigosa

Criação, edição e exclusão são realmente limitadas ao owner/supremo na API CMS (`cms/backend/src/services/authService.ts:33-45`, `cms/backend/src/services/authService.ts:103-174`, `cms/backend/src/services/authService.ts:216-229`). O CMS, porém, descreve o perfil “Administrador” como alguém que pode criar outros usuários (`cms/frontend/src/app/developer/usuarios/page.tsx:351-368`), o que não é verdade para admins comuns. A mensagem posterior da mesma página informa corretamente que somente o supremo gerencia acessos (`cms/frontend/src/app/developer/usuarios/page.tsx:645-660`).

Os campos de edição de nome/e-mail não têm os limites presentes na criação e podem ser truncados/normalizados na API CMS sem contador (`cms/frontend/src/app/developer/usuarios/page.tsx:326-348`, `cms/frontend/src/app/developer/usuarios/page.tsx:534-570`, `cms/backend/src/services/authService.ts:176-188`).

No contrato HTTP, `updateUser` transforma `role` ausente em `admin` e usa `Boolean(params.active)`, fazendo por exemplo a string `"false"` virar `true` (`cms/backend/src/services/authService.ts:176-188`). A UI atual envia os quatro campos tipados, então o fluxo visual normal funciona; ainda assim a borda da API CMS deveria validar `unknown` explicitamente para evitar promoção/coerção em chamadas parciais.

Os testes de auth existentes cobrem rate limit de login, não CRUD, owner, invalidação de sessão nem essas coerções (`cms/backend/tests/authService.test.ts:17-64`).
...
1. Termos — “Uso responsável” e seu parágrafo (`site/frontend/src/app/termos-de-uso/page.tsx:72-79`).
2. Termos — aside “Privacidade e dados” e seu parágrafo (`site/frontend/src/app/termos-de-uso/page.tsx:82-92`).
3. Privacidade — seção “Resumo rápido”, título, descrição e corpo (`site/frontend/src/app/privacidade/page.tsx:60-72`).
4. Privacidade — aside “LGPD e transparência” e seu parágrafo (`site/frontend/src/app/privacidade/page.tsx:74-86`).
...
Os 42 aliases públicos declarados em `site/frontend/src/lib/routes.ts:258-305` possuem as mesmas origens e destinos dos redirects de `site/frontend/next.config.js:134-182`; o rewrite adicional de uploads é esperado. Porém, os redirects estão duplicados manualmente em duas fontes, apesar de `routes.ts` ser documentado como fonte canônica. Não há divergência atual, mas há risco direto de drift em futura alteração.
...
- Fetch público de conteúdo, SEO e slots usa `cache: "no-store"`, então uma alteração válida aparece sem cache stale (`site/frontend/src/lib/api.ts:76-83`, `site/frontend/src/lib/cmsPublic.ts:18-23`, `site/frontend/src/lib/cmsPublic.ts:73-78`).

Riscos e custos:

- `listAdminImages` percorre e executa `stat` em todo `site/frontend/public` a cada carga da biblioteca; o custo cresce linearmente com arquivos (`cms/backend/src/services/mediaService.ts:214-296`).
...
Comando executado na API CMS:

```text
cd cms/backend && cmd /c npm test -- --run tests/consentService.test.ts tests/authService.test.ts tests/trackingService.test.ts
```
