...
const FIELD_SECTIONS: Array<{
...
}> = [
  {
    ...
    description: "Esses textos aparecem logo no topo do contato público.",
    fields: [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "phoneHours", label: "Horário de atendimento" },
  ...
  {
    title: "Endereço e CTA",
    description: "Informações institucionais e botão final da página.",
    fields: [
      { key: "addressLine", label: "Endereço completo", span: "full" },
...
export default function ContatoInfoPage() {
  ...
  async function handleSave() {
    ...
    if (requiredValues.some((value) => !String(value).trim())) {
      setStatus("error");
      setStatusMessage("Preencha os campos principais antes de salvar.");
      return;
    }
  ...
  return (
    <DeveloperPage>
      <DeveloperHero
        eyebrow="Conteúdo - Contato"
        title="Textos e dados da página de contato."
        description="A configuração foi trazida da versão estática, agora ligada ao site atual e ao storage de site texts."
        stats={[
      ...
      {loading ? (
        <div className="mt-6">
          <DeveloperMessage tone="info">Carregando configuração de contato...</DeveloperMessage>
        </div>
      ) : null}
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <DeveloperCard>
          <DeveloperSectionHeading
            ...
            title="Editar informações institucionais"
            description="As alterações abaixo alimentam a página /fale-conosco do projeto React atual."
          />
