...
export default function SobreHeroPage() {
  ...
  async function handleSave() {
    if (
      ...
      !form.subtitle.trim() ||
      !form.image.trim() ||
      form.stats.some((item) => !item.number.trim() || !item.description.trim())
    ) {
      setStatus("error");
      setStatusMessage("Preencha todos os campos obrigatorios antes de salvar.");
      return;
    }
    ...
    setStatus("success");
    setStatusMessage("Hero da página sobre salvo com sucesso.");
    await refresh();
  }
  ...
  return (
    <DeveloperPage>
      <DeveloperHero
        eyebrow="Conteúdo - Sobre"
        title="Hero institucional e números da página sobre."
        description="A configuração reaproveita o contrato antigo de site texts, agora refletido na página React atual."
        stats={[
      ...
      {loading ? (
        <div className="mt-6">
          <DeveloperMessage tone="info">Carregando hero da página sobre...</DeveloperMessage>
        </div>
      ) : null}
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
        <DeveloperCard>
          <DeveloperSectionHeading
            eyebrow="Formulário principal"
            title="Editar hero da página sobre"
            description="Tag, título, subtítulo, imagem principal e os três números em destaque."
          />
          ...
          <div className="space-y-5">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <DeveloperImageField
                ...
                previewAlt={form.title || "Preview do hero sobre"}
              />
            </div>
            ...
            <DeveloperField label="Subtítulo" required>
              <textarea
                rows={4}
                value={form.subtitle}
                onChange={(event) =>
              ...
              <div className="mt-4 space-y-4">
                {form.stats.map((item, index) => (
                  <div key={index} className="grid gap-4 sm:grid-cols-2">
                    <DeveloperField label={`Número ${index + 1}`} required>
                      <input
                        value={item.number}
                    ...
                    <DeveloperField label={`Descrição ${index + 1}`} required>
                      <input
        ...
        <DeveloperCard>
          <DeveloperSectionHeading
            ...
            description="Conferência rápida do conteúdo que alimenta o topo da página sobre."
          />
