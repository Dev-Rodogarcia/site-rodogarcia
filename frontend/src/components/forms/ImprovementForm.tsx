...
export default function ImprovementForm({ profile: fixedProfile, endpoint = api.forms.improvements, internal = false, onSubmitted }: ImprovementFormProps) {
  ...
  useEffect(() => {
    if (!internal) return;
    setInternalName((value) => value || session?.user?.name || "");
    setInternalEmail((value) => value || session?.user?.email || "");
  }, [internal, session?.user?.email, session?.user?.name]);

  useEffect(() => {
    if (!internal) return;
    let active = true;
    void apiRequest<{ users?: UserSuggestion[] }>(api.admin.users).then((response) => {
      if (active && response.success) setUserSuggestions(response.data?.users ?? []);
    });
    return () => { active = false; };
  }, [apiRequest, internal]);
  ...
  function selectKnownUser(value: string, field: "name" | "email") {
    const match = userSuggestions.find((user) => normalized(field === "name" ? user.name ?? "" : user.email) === normalized(value));
    if (field === "name") setInternalName(value);
    else setInternalEmail(value);
    if (!match) return;
    if (field === "name") setInternalEmail(match.email);
    else setInternalName(match.name ?? "");
  }
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const data = internal
        ? await apiRequest<{ message?: string }>(endpoint, { method: "POST", body: payload })
        : await fetch(endpoint, { method: "POST", body: payload }).then(async (response) => {
  ...
  if (status === "success") return <SuccessState internal={internal} isEmployee={isEmployee} message={message} onReset={() => { setStatus("idle"); setSelectedProfile(fixedProfile ?? null); }} />;

  return <section data-profile={profile ?? "selection"} className={cn("rounded-[24px] border p-5 shadow-[0_22px_56px_rgba(15,23,42,0.1)] transition-colors duration-500 sm:p-8", !internal && "h-full rounded-[32px]", profile ? (isEmployee ? "border-emerald-200 bg-[linear-gradient(180deg,rgba(236,253,245,0.98),rgba(236,253,245,0.72))]" : "border-blue-200 bg-[linear-gradient(180deg,rgba(239,246,255,0.98),rgba(239,246,255,0.72))]") : "border-white bg-[linear-gradient(180deg,rgba(255,255,255,0.95),rgba(241,245,249,0.92))]")}>{!profile ? <ProfileSelector onSelect={setSelectedProfile} /> : <div key={profile} className="animate-in fade-in-0 slide-in-from-bottom-3 duration-500"><form onSubmit={submit} className="flex h-full flex-col space-y-5" noValidate>
    <FormHeading isEmployee={isEmployee} showProfileChooser={!fixedProfile} onChooseProfile={() => setSelectedProfile(null)} />
    <div className="grid gap-4 sm:grid-cols-2">
      <Field label="Nome" required><input name="name" required autoComplete="name" list={internal ? "internal-user-names" : undefined} value={internal ? internalName : undefined} defaultValue={internal ? undefined : undefined} onChange={internal ? (event) => selectKnownUser(event.target.value, "name") : undefined} className={fieldClassName} placeholder="Seu nome" /></Field>
      <Field label="E-mail" required><input name="email" required type="email" autoComplete="email" list={internal ? "internal-user-emails" : undefined} value={internal ? internalEmail : undefined} defaultValue={internal ? undefined : undefined} onChange={internal ? (event) => selectKnownUser(event.target.value, "email") : undefined} className={fieldClassName} placeholder="voce@empresa.com" /></Field>
    </div>
    {internal && isEmployee ? <InternalEmployeeFields categories={categories} /> : isEmployee ? <PublicEmployeeFields categories={categories} phone={phone} onPhoneChange={setPhone} /> : <SiteUserFields categories={categories} />}
    ...
    <AttachmentPicker attachments={attachments} onSelect={selectAttachments} onRemove={(index) => setAttachments((current) => current.filter((_, currentIndex) => currentIndex !== index))} inputRef={inputRef} theme={theme} submitAction={internal ? <SubmitButton status={status} isEmployee={isEmployee} /> : undefined} />
    ...
    {internal ? <><datalist id="internal-user-names">{userSuggestions.filter((user) => user.name).map((user) => <option key={user.email} value={user.name} />)}</datalist><datalist id="internal-user-emails">{userSuggestions.map((user) => <option key={user.email} value={user.email} />)}</datalist></> : null}
    {!internal ? <div className="mt-auto flex justify-end"><SubmitButton status={status} isEmployee={isEmployee} /></div> : null}
  </form></div>}</section>;
}
...
function InternalEmployeeFields({ categories }: { categories: readonly (readonly [string, string])[] }) { return <div className="grid gap-4 lg:grid-cols-2"><div className="space-y-4"><Field label="Área"><input name="area" className={fieldClassName} placeholder="Ex.: Operação" /></Field><CategoryField categories={categories} /></div><Field label="Contexto e impacto" required><textarea name="message" required minLength={10} rows={5} className={`${fieldClassName} resize-y`} placeholder="Como é feito hoje e qual dificuldade essa melhoria resolve?" /></Field></div>; }
function PublicEmployeeFields({ categories, phone, onPhoneChange }: { categories: readonly (readonly [string, string])[]; phone: string; onPhoneChange: (value: string) => void }) { return <><div className="grid gap-4 sm:grid-cols-2"><Field label="Telefone"><input name="phone" value={phone} onChange={(event) => onPhoneChange(phoneMask(event.target.value))} type="tel" inputMode="numeric" autoComplete="tel" maxLength={15} className={fieldClassName} placeholder="(00) 00000-0000" /></Field><Field label="Filial" required><select name="branch" required defaultValue="" className={fieldClassName}><option value="" disabled>Selecione a filial</option>{branches.map((branch) => <option key={branch} value={branch}>{branch}</option>)}</select></Field><Field label="Área"><input name="area" className={fieldClassName} placeholder="Ex.: Operação" /></Field><CategoryField categories={categories} /></div><Field label="Contexto e impacto" required><textarea name="message" required minLength={10} rows={5} className={`${fieldClassName} resize-y`} placeholder="Como é feito hoje e qual dificuldade essa melhoria resolve?" /></Field></>; }
function SiteUserFields({ categories }: { categories: readonly (readonly [string, string])[] }) { return <><div className="grid gap-4 sm:grid-cols-2"><CategoryField label="Sobre o que é?" categories={categories} /><Field label="Página do site, se souber"><input name="page" className={fieldClassName} placeholder="Ex.: /cotacao" /></Field></div><Field label="Conte sua sugestão ou o problema" required><textarea name="message" required minLength={10} rows={5} className={`${fieldClassName} resize-y`} placeholder="Descreva o que aconteceu ou o que você sugere." /></Field></>; }
...
function AttachmentPicker({ attachments, onSelect, onRemove, inputRef, theme, submitAction }: { attachments: File[]; onSelect: (files: FileList | null) => void; onRemove: (index: number) => void; inputRef: React.RefObject<HTMLInputElement | null>; theme: "blue" | "emerald"; submitAction?: React.ReactNode }) { const blue = theme === "blue"; return <div className="rounded-2xl border border-dashed border-[var(--border)] bg-white/72 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-medium text-[var(--foreground)]">Anexos <span className="font-normal text-[var(--color-muted-raw)]">(opcional)</span></p><p className="mt-1 text-xs leading-5 text-[var(--color-muted-raw)]">Fotos, CSV, XLS ou XLSX · até 5 arquivos de 8 MB cada.</p></div><div className="flex flex-wrap items-center gap-3"><button type="button" onClick={() => inputRef.current?.click()} className={cn("inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-semibold", blue ? "border-blue-200 text-blue-700 hover:bg-blue-50" : "border-emerald-200 text-emerald-700 hover:bg-emerald-50")}><FileArrowUp size={16} weight="bold" />Adicionar arquivos</button>{submitAction}</div><input ref={inputRef} type="file" accept={acceptedFiles} multiple className="sr-only" onChange={(event) => onSelect(event.target.files)} /></div>{attachments.length ? <ul className="mt-3 flex flex-wrap gap-2">{attachments.map((file, index) => <li key={`${file.name}-${index}`} className="inline-flex max-w-full items-center gap-2 rounded-lg bg-slate-100 px-2.5 py-1.5 text-xs text-slate-700"><span className="truncate">{file.name}</span><button type="button" aria-label={`Remover ${file.name}`} onClick={() => onRemove(index)} className="text-slate-500 hover:text-red-600"><X size={14} weight="bold" /></button></li>)}</ul> : null}</div>; }
