...
export default function ImprovementForm({ profile: fixedProfile, endpoint = api.forms.improvements, internal = false, onSubmitted, onProfileChange }: ImprovementFormProps) {
  ...
  if (status === "success") return <SuccessState internal={internal} isEmployee={isEmployee} message={message} onReset={() => { setStatus("idle"); changeProfile(fixedProfile ?? null); }} />;
  ...
  return <section data-profile={profile ?? "selection"} className={cn("rounded-[24px] border p-5 shadow-[0_22px_56px_rgba(15,23,42,0.1)] transition-colors duration-500 sm:p-8", !internal && "h-full rounded-[32px]", profile ? (isEmployee ? "border-emerald-200 bg-[linear-gradient(180deg,rgba(236,253,245,0.98),rgba(236,253,245,0.72))]" : "border-blue-200 bg-[linear-gradient(180deg,rgba(239,246,255,0.98),rgba(239,246,255,0.72))]") : "border-white bg-[linear-gradient(180deg,rgba(255,255,255,0.95),rgba(241,245,249,0.92))]")}>{!profile ? <ProfileSelector onSelect={changeProfile} /> : <div key={profile} className="animate-in fade-in-0 duration-300"><form onSubmit={submit} className="flex h-full flex-col space-y-4" noValidate>
    ...
    {!isEmployee && !internal ? <SiteUserFormFields categories={categories} /> : <><div className={cn("grid gap-4 sm:grid-cols-2", isEmployee && !internal && "lg:grid-cols-3")}>
      ...
      {isEmployee && !internal ? <Field label="Telefone"><input name="phone" value={phone} onChange={(event) => setPhone(phoneMask(event.target.value))} type="tel" inputMode="numeric" autoComplete="tel" maxLength={15} className={fieldClassName} placeholder="(00) 00000-0000" /></Field> : null}
    </div>
    {internal && isEmployee ? <InternalEmployeeFields categories={categories} /> : isEmployee ? <PublicEmployeeFields categories={categories} /> : null}</>}
    {internal && isEmployee ? <div className="grid gap-4 sm:grid-cols-2"><Field label="Resultado esperado"><input name="expectedResult" className={fieldClassName} placeholder="Como seria a solução ideal?" /></Field><Field label="Onde será aplicado"><input name="applicationPlace" className={fieldClassName} placeholder="Ex.: rotina de coleta" /></Field></div> : null}
