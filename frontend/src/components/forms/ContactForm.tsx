...
export default function ContactForm() {
    ...
    return (
      <div className="rounded-[32px] border border-white/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.9)_0%,rgba(241,245,249,0.96)_100%)] p-8 text-center shadow-[0_24px_56px_rgba(15,23,42,0.08)]">
        ...
        <p className="mt-3 text-sm leading-7 text-[var(--color-muted-raw)]">
          A equipe recebeu sua solicitação e responde em até 1 dia útil.
        </p>
        <button
          type="button"
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h3 className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-[var(--foreground)]">
            Fale com a equipe pelo canal institucional.
          </h3>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <input
            {...register("subject")}
            type="text"
            placeholder="Ex: Dúvida sobre rastreio"
            className={fieldClassName}
          />
        </div>
      </div>
      ...
      <div>
        <label className="mb-1.5 block text-sm font-medium text-[var(--foreground)]">
          Mensagem <span className="text-[var(--primary)]">*</span>
        </label>
        <textarea
          {...register("message")}
          rows={5}
          ...
          className={`${fieldClassName} resize-none ${
            errors.message ? "border-red-500 focus:ring-red-500/10" : ""
          }`}
        />
        {errors.message ? (
