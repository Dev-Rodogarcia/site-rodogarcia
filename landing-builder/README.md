...
- `backend/`: API privada em `127.0.0.1:36110` no DEV e `41110` em produção. O CMS é o único consumidor dos endpoints internos e se autentica com `LANDING_BUILDER_SERVICE_TOKEN`.
...
`iniciar-dev.bat` inicia o backend e o frontend do Builder nas portas `36110` e `35112`. Em produção, `iniciar-prod.bat` instala, valida, empacota, promove e supervisiona os dois processos em `41110` e `41112` pelo PM2; o pre-flight falha se `LANDING_BUILDER_SERVICE_TOKEN` ou `LANDING_BUILDER_STORAGE_ROOT` não estiverem configurados de forma segura.
