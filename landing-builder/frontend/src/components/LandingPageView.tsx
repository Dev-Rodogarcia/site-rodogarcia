...
function Cta({ label, href, color, textColor }: { label: string; href: string; color: string; textColor: string }) {
  ...
  return (
    <a href={href} style={{ display: "inline-block", marginTop: 28, padding: "14px 22px", borderRadius: 8, background: color, color: textColor, fontWeight: 700 }}>
      {label}
    </a>
  );
}
...
export function LandingPageView({ landing, preview = false }: { landing: PublicLandingPage; preview?: boolean }) {
  ...
  return (
    <main style={{ background: theme.backgroundColor, color: theme.textColor, fontFamily: fontFamilies[theme.font] }}>
      {preview ? null : <LandingAnalytics measurementId={landing.analytics.ga4MeasurementId} />}
      <section
        style={{
          minHeight: "68vh",
          color: heroTextColor,
          background: hasBackgroundImage ? theme.primaryColor : theme.backgroundColor,
          backgroundImage: hasBackgroundImage ? `linear-gradient(90deg, rgba(4,11,25,.86), rgba(4,11,25,.2)), url(${landing.hero.backgroundImage})` : undefined,
          backgroundSize: "cover",
          backgroundPosition: "center",
          padding: "0 max(24px, 8vw) 72px",
        }}
      >
        <div style={{ display: "flex", justifyContent: "space-between", gap: 16, padding: "13px 0", fontSize: 14, borderBottom: `1px solid ${hasBackgroundImage ? "rgba(255,255,255,.18)" : "rgba(17,17,17,.16)"}` }}>
          <span>{landing.hero.phone}</span>
          <span>{landing.hero.email}</span>
        </div>
        <div style={{ minHeight: "56vh", display: "grid", alignContent: "center" }}>
          ...
          <div style={{ maxWidth: 760 }}>
            ...
            <h1 style={{ fontSize: "clamp(2.6rem, 7vw, 5.5rem)", lineHeight: 0.95, margin: "18px 0" }}>{landing.hero.title}</h1>
          ...
          {landing.hero.highlights.length > 0 ? (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 14, marginTop: 34, maxWidth: 1000 }}>
              {landing.hero.highlights.map((item, index) => (
                <article key={`${item.title}-${index}`} style={{ padding: 18, border: `1px solid ${hasBackgroundImage ? "rgba(255,255,255,.56)" : "rgba(17,17,17,.2)"}`, borderRadius: 10, background: hasBackgroundImage ? "rgba(8,16,28,.5)" : theme.backgroundColor, backdropFilter: hasBackgroundImage ? "blur(8px)" : undefined }}>
                  <strong>{item.title}</strong>
                  <p style={{ margin: "8px 0 0", fontSize: 13, lineHeight: 1.45, color: mutedHeroText }}>{item.description}</p>
                </article>
              ))}
            </div>
          ) : null}
      ...
      <section style={{ padding: "84px max(24px, 8vw)", maxWidth: 1280, margin: "0 auto" }}>
        ...
        <Cta label={landing.lowerSection.ctaLabel} href={landing.lowerSection.ctaUrl} color={theme.primaryColor} textColor="#fff" />
      </section>
