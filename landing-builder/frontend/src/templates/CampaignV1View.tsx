...
function CampaignMedia({ landing, url, fallbackAlt, style }: {
  ...
  const description = descriptor?.alt || fallbackAlt;
  if (descriptor?.kind === "video") {
    return <video controls playsInline preload="metadata" poster={descriptor.poster || undefined} aria-label={description} style={style}>
      <source src={url} />
      Seu navegador não suporta este vídeo.
    </video>;
  }
  return <img src={url} alt={description} style={style} />;
}
  ...
  return <main style={{ background: theme.backgroundColor, color: theme.textColor, fontFamily: fontFamilies[theme.font] }}>
    ...
    <section style={{ minHeight: "68vh", color: heroTextColor, background: hasBackgroundImage ? theme.primaryColor : theme.backgroundColor, backgroundImage: hasBackgroundImage ? `linear-gradient(90deg, rgba(4,11,25,.86), rgba(4,11,25,.2)), url(${landing.hero.backgroundImage})` : undefined, backgroundSize: "cover", backgroundPosition: "center", padding: `0 ${contentPadding} 72px` }}>
      ...
      <div style={{ minHeight: "56vh", display: "grid", alignContent: "center" }}>
        {landing.hero.logo ? <CampaignMedia landing={landing} url={landing.hero.logo} fallbackAlt={landing.name} style={{ width: 210, maxWidth: "60%", marginBottom: 48 }} /> : <strong style={{ fontSize: 26, letterSpacing: ".06em", marginBottom: 48 }}>SUA LOGO</strong>}
    ...
    {landing.finalCta.visible ? <section style={{ borderTop: sectionDivider, padding: `28px ${contentPadding}`, background: theme.backgroundColor }}><div style={{ maxWidth: 1480, minHeight: 300, margin: "0 auto", padding: "clamp(42px, 6vw, 80px)", borderRadius: 28, color: "#ffffff", textAlign: "left", background: theme.primaryColor, backgroundImage: landing.finalCta.backgroundImage ? `linear-gradient(90deg, rgba(0,0,0,.82), rgba(0,0,0,.58)), url(${landing.finalCta.backgroundImage})` : undefined, backgroundSize: "cover", backgroundPosition: "center" }}><div style={{ maxWidth: 840 }}><Eyebrow color="rgba(255,255,255,.78)">{landing.finalCta.eyebrow}</Eyebrow><h2 style={{ maxWidth: 760, fontSize: "clamp(2.2rem, 5vw, 4.4rem)", lineHeight: .98, letterSpacing: "-.05em", margin: "16px 0" }}>{landing.finalCta.title}</h2><p style={{ maxWidth: 660, fontSize: "1.1rem", lineHeight: 1.7, opacity: .88 }}>{landing.finalCta.description}</p><Cta label={landing.finalCta.ctaLabel} href={landing.finalCta.ctaUrl} color="#ffffff" textColor={theme.primaryColor} /></div></div></section> : null}
